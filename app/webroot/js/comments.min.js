...
$(function () {
    ...
    $(document).on('click', '.btnReplyComment', function () {
        ...
        var comment_post_id = $(this).data('id');
        $('.div-post-reply').html('');
        $('#div-post-reply-' + comment_post_id).html(html);
        $('#btnSendReply').data('id', comment_post_id);
        $('#inputReply').focus();
    });
    //Nút gửi trả lời
    $(document).on('click', '#btnSendReply', function () {
        ...
        {
            $.ajax({
                url: '/commentpostreplies/send_reply',
                type: 'post',
                dataType: 'json',
                data: {
                    reply: reply,
...
function load_comment(page, post_id) {
    $.ajax({
        url: '/commentposts/get_comment',
        type: 'post',
        dataType: 'json',
        data: {
            page: page,
        ...
        success: function (dt) {
            if(dt.data != null)
            {
                ...
                if(dt.sum_remain > 0)
                {
                    $('#div-more-comment').html('<a href="javascript: void(0)" id="a-more-comment" data-page="' + (page + 1) + '" data-post_id="' + post_id + '">Xem thêm +</a>')
                }
                else
                {
                                ...
                                <div class="widget-box transparent">\
                                    ...
                                    <div class="widget-body">\
                                        <div class="widget-main">\
                                            ...
                                            <div class="div-post-reply" id="div-post-reply-' + id + '"></div>\
                                        </div>\
                                    </div>\
                                </div>\
                            </div>\
