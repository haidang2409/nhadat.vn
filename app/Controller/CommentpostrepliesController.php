<?php
class CommentpostrepliesController extends AppController
{
    ...
    function send_reply()
    {
        ...
        if($this->Session->check('Member'))
        {
            if($this->request->is('post') || $this->request->is('put'))
            {
                ...
                $this->Commentpostreply->set('member_id', $member_id);
                $this->Commentpostreply->set('comment_post_id', $id);
                $this->Commentpostreply->set('reply', $reply);
                if($this->Commentpostreply->save())
                {
                    ...
                    $this->Commentpostreply->recursive = -1;
                    $replypost = $this->Commentpostreply->findById($this->Commentpostreply->id);
                    $data = array(
                        'status' => 'success',
                        'reply' => nl2br(htmlentities($replypost['Commentpostreply']['reply'], ENT_QUOTES, 'UTF-8')),
                        'reply_id' => $replypost['Commentpostreply']['id'],
                        'created' => $this->Library->time_elapsed_string($replypost['Commentpostreply']['created']),
    ...
    function like()
    {
        ...
        if($this->Session->check('Member'))
        {
            if($this->request->is('post'))
            {
                ...
                $this->Commentpostreply->recursive = -1;
                if($this->Commentpostreply->updateAll(array('Commentpostreply.like' => 'Commentpostreply.like + 1'), array('Commentpostreply.id' => $reply_id)))
                {
                    $this->Commentpostreply->recursive = -1;
                    $like = $this->Commentpostreply->find('first', array('conditions' => array('id' => $reply_id)));
                    echo json_encode(array(
                        'status' => 'success',
                        'like' => $like['Commentpostreply']['like']
                    ));
                }
            }
        }
