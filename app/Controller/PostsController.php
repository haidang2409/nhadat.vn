...
class PostsController extends AppController
{
    ...
    function index()
    {
        ...
        {
            $conditions_category = 'Postcat.id = "' . substr($postcat_id, 2) . '" AND Postcat.link = "' . $postcat_link . '"';
        }
        //Breadcrumb cat
        $this->Post->Postcat->recursive = -1;
        $breadcrumb_postcat = $this->Post->Postcat->findById(substr($postcat_id, 2));
        $title = 'Tin tức bất động sản';
        if($breadcrumb_postcat)
        {
            $title = $breadcrumb_postcat['Postcat']['name'];
            $this->set(array('breadcrumb_postcat' => $breadcrumb_postcat));
        }
        $postcats = $this->Post->Postcat->find('all');
        ...
        $this->paginate = array(
            'joins' => array(
                ...
                array(
                    ...
                    'type' => 'INNER',
                    'foreignKey' => false,
                    'conditions' => 'Post.post_category_id = Postcat.id'
                ),
            ),
            'fields' => array('*'),
            'conditions' => array(
...
//            $this->Session->setFlash('Không tìm thấy trang theo yêu cầu', 'flashWarning');
    ...
    function admin_index()
    {
        ...
        $categories = null;
        $this->Post->Postcat->recursive = -1;
        $category = $this->Post->Postcat->find('all');
        foreach ($category as $item) {
            $categories[$item['Postcat']['id']] = $item['Postcat']['name'];
        }
        ...
        $condition_category = $category_search != ''? 'Postcat.id =' . $category_search: '';
        //
        $this->Post->recursive = -1;
        $this->paginate = array(
            'joins' => array(
    ...
    function admin_add()
    {
        ...
        $postcats = null;
        $this->Post->Postcat->recursive = -1;
        $postcat = $this->Post->Postcat->find('all');
        foreach ($postcat as $item)
        {
            $postcats[$item['Postcat']['id']] = $item['Postcat']['name'];
        }
        $this->set(array(
            'title' => 'Thêm bài viết',
            'postcats' => $postcats
